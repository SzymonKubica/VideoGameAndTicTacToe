...
public class GameTree implements GameTreeInterface {
  ...
  private void expandTree(GameTreeNode node) {
    ...
    if (!currentBoard.isFinished()) {
      ...
      for (int i = 1; i <= 9; i++) {
        if (currentBoard.getMark(i) == Board.EMPTY) {
          GameTreeNode newNode;
          Board newBoard = currentBoard.makeCopy();
          if (currentBoard.getMark(currentBoard.getLastMarkPosition()) == Board.CROSS) {
            newBoard.setMark(i, Board.NOUGHT);
          } else {
            newBoard.setMark(i, Board.CROSS);
          }
  ...
  public int[] BestMoves() {
    ...
    for (GameTreeNode nextBoard : children) {
